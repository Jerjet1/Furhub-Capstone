...
export default function BookingList() {
  ...
  const moveIndicator = (index: number) => {
    ...
    Animated.spring(indicatorX, { toValue: offset, useNativeDriver: true }).start();
  };
      ...
      <View style={{ zIndex: 1, backgroundColor: "#fff" }}>
        ...
        >
          {categories.map((cat, index) => (
            <TouchableOpacity
              key={cat.id}
              style={[styles.category, cat.name === selectedCategory && styles.activeCategory]}
              onPress={() => onTabPress(index, cat.name)}
              onLayout={(e) => (tabWidths.current[index] = e.nativeEvent.layout.width)}
            >
              <Text
                style={[styles.categoryText, cat.name === selectedCategory && styles.activeCategoryText]}
              >
                {cat.name}
              </Text>
            </TouchableOpacity>
          ...
          <Animated.View
            style={[
              styles.underline,
              {
                width: tabWidths.current[categories.findIndex((c) => c.name === selectedCategory)] || 0,
...
const styles = StyleSheet.create({
  category: { backgroundColor: "#F9F9F9", paddingVertical: 8, paddingHorizontal: 15, borderRadius: 20, marginRight: 10 },
  activeCategory: { backgroundColor: "#EAF4FF" },
  categoryText: { fontSize: 13, color: "#666" },
  activeCategoryText: { color: "#007AFF", fontWeight: "600" },
  underline: { height: 3, backgroundColor: "#007AFF", position: "absolute", bottom: 0, left: 15, borderRadius: 2 },
  card: { flexDirection: "row", backgroundColor: "#fff", borderRadius: 12, padding: 12, marginHorizontal: 15, marginBottom: 15, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  ...
  cancelButton: { backgroundColor: "#e74c3c", padding: 8, borderRadius: 12, marginRight: 10 },
